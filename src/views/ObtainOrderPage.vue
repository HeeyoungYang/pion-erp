                  ...
                  <v-row class="mt-5" justify="space-between">
                    <v-col align-self="center" cols="12" sm="5" class="pb-0">
                      ...
                      <table style=" border-spacing: 0px; width: 100%;" class="mt-1">
                        ...
                        <tr>
                          <td class="estimate_info estimate_title text-center" style="border-bottom: 0px;border-left:1px solid #b6b6b6">{{ type_obtain === '용역' ? '용역명' : '프로젝트명' }}</td>
                          <td class="estimate_info" style="border-bottom: 0px;">{{ input_service_name2.value }}</td>
                        </tr>
                        <tr>
                          <td class="estimate_info estimate_title text-center" style="border-bottom: 0px;border-left:1px solid #b6b6b6">{{ type_obtain === '용역' ? '용역기간' : '프로젝트기간' }}</td>
                          <td class="estimate_info" style="border-bottom: 0px;">{{ input_service_period2.value }}</td>
                        </tr>
                        <tr>
                          <td class="estimate_info estimate_title text-center" style="border-left:1px solid #b6b6b6">유효기간</td>
                          <td class="estimate_info"> 발행일로부터 30일 이내 </td>
...
export default {
  ...
  watch:{
    ...
    during_edit() {
      this.estimateFilesInputs = JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateFilesInputs));
    },
  ...
  computed: {
    ...
    input_obtain_file() { return this.estimateFilesInputs.find(x=>x.column_name === 'obtain')},
    ...
    input_obtain_file2() { return this.estimateWriteFilesInputs.find(x=>x.column_name === 'obtain')},
  ...
  methods:{
    ...
    async saveLabor() {
      ...
      if(this.tab_main === 0){
        ...
        let sendData = {
          ...
          "obtain_labor_cost_calc_detail_table-delete": [{
            "user_info": {
              "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
              "role": "modifier"
            },
          ...
          "obtain_labor_cost_calc_detail_table-insert": []
        };
        this.labor_cost_data.forEach(data => {
          data.cost_calc_code = new_cost_calc_code;
          sendData["obtain_labor_cost_calc_detail_table-insert"].push({
            "user_info": {
              "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
              "role": "creater"
            },
      ...
      if(validate) {
        ...
        if (this.input_obtain_file.value){
          ...
          sendData.files.push({
            ...
            "name": this.input_obtain_file.value.name
          });
        }
        ...
        if (this.input_etc_file.value && this.input_etc_file.value.length > 0){
          sendData['obtain_confirmation_table-update'][0].etc_files = new_cost_calc_code + '_' + this.input_etc_file.value.map(x=>x.name).join('/');
        ...
        try {
          ...
          if(result['code'] == 0 || (typeof result['data'] === 'object' && result['data']['code'] == 0) || (typeof result['response'] === 'object' && typeof result['response']['data'] === 'object' && result['response']['data']['code'] == 0)){
            ...
            const newObtainFile = this.estimateFilesInputs.find(x=>x.column_name === 'obtain').value;
            ...
            if (newObtainFile){
              targetConfirmation.obtain_file = `${new_cost_calc_code + '_' + newObtainFile.name}`;
              targetConfirmation.obtain_thumbnail = `${mux.Util.uint8ArrayToHexString(await mux.Util.getPdfThumbnail(newObtainFile, 1, false, 1000, 1000))}`;
            }
            if (newBlueprintFile){
            ...
            if (newObtainFile){
              this.clickedProductCost.obtain_file = new_cost_calc_code + '_' + newObtainFile.name;
              this.clickedProductCost.obtain_thumbnail = mux.Util.uint8ArrayToHexString(await mux.Util.getPdfThumbnail(newObtainFile, 1, false, 1000, 1000));
            }
            if (newBlueprintFile){
    ...
    async surveyCostNumEditSave() {
      ...
      if(validate) {
        ...
        let sendData = {
          ...
          "obtain_cost_calc_detail_table-delete": [{
            "user_info": {
              "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
              "role": "modifier"
            },
          ...
          "obtain_construction_detail_table-delete": [{
            "user_info": {
              "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
              "role": "modifier"
            },
          ...
          "obtain_cost_calc_detail_table-insert": [],
          "obtain_construction_detail_table-insert": [],
        };
        ...
        if (this.clickedProductCost.obtain_type !== '기술료'){
          ...
          if (product_cost_materials){
            ...
            new_product_cost_calc_detail.forEach(data => {
              sendData["obtain_cost_calc_detail_table-insert"].push({
                "user_info": {
                  "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
                  "role": "creater"
                },
          ...
          if (product_cost_construction_materials){
            ...
            new_construction_materials_data.forEach(data => {
              sendData["obtain_construction_detail_table-insert"].push({
                "user_info": {
                  "user_id": this.$cookies.get(this.$configJson.cookies.id.key),
                  "role": "creater"
                },
    ...
    async save() {
      ...
      if (!this.input_obtain_file2.value){
      ...
      let sendData = {
        "obtain_confirmation_table-insert": [{
          ...
          "data":{
            ...
            obtain_file: new_cost_calc_code + '_' + this.input_obtain_file2.value.name,
            obtain_thumbnail: mux.Util.uint8ArrayToHexString(await mux.Util.getPdfThumbnail(this.input_obtain_file2.value, 1, false, 1000, 1000)),
      ...
      if (this.input_obtain_file2.value){
        sendData.files.push({
          folder: 'obtain/obtain',
          file: this.input_obtain_file2.value,
          name: this.input_obtain_file2.value.name
        });
      }
      if (this.input_blueprint_file2.value){
        sendData.files.push({
          folder: 'obtain/blueprint',
  ...
  data(){
    return{
      ...
      searchCardInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.searchCardInputs)),
      estimateSearchDefaultInfoInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateSearchDefaultInfoInputs)),
      estimateSearchDefaultInfoInputs2: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateSearchDefaultInfoInputs2)),
      estimateSearchCompanyInfoInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateSearchCompanyInfoInputs)),
      estimateWriteDefaultInfoInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateWriteDefaultInfoInputs)),
      estimateWriteDefaultInfoInputs2: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateWriteDefaultInfoInputs2)),
      estimateWriteCompanyInfoInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateWriteCompanyInfoInputs)),
      estimateFilesInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateFilesInputs)),
      estimateWriteFilesInputs: JSON.parse(JSON.stringify(ObtainOrderPageConfig.estimateWriteFilesInputs)),
