                    ...
                    <v-card class="pa-0">
                      <v-list class="pa-3">
                        ...
                        <v-btn
                          ...
                          color="primary"
                          class="mt-3"
                          @click="saveUserAuthority(item.user_id, item.authCheck)"
                        >저장</v-btn>
                      </v-list>
                    </v-card>
                  </v-menu>
...
export default {
  ...
  methods: {
    ...
    saveUserAuthority(user_id, checked_list){
      this.$emit("changeAuthority", {user_id: user_id, authCheck: checked_list});
    ...
    printInboundApprove(refs,fileName){
      setTimeout(async () => {
        if (fileName){
          await mux.Util.downloadPDF(refs, {fileName, countingTableIndex: 2, rowCountPerPage: 20, trTdClass: 'approve_text'});
        }else {
          await mux.Util.print(refs, {countingTableIndex: 2, rowCountPerPage: 20, trTdClass: 'approve_text'});
        }
      }, 500);
    },
