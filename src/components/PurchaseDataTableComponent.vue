            ...
            <template v-slot:item="{ index, item, select, isSelected, expand }">
              <tr  @click="clickTr(item)">
                <td align="center">
                  ...
                  <v-chip
                    v-if="item.approval_phase == '요청'  && item.checker_id !== userID"
                    class="ma-2"
                    small
                    :color="item.approval_phase == '요청' ? 'default' : 'amber lighten-4'"
                  >
                    {{ item.approval_phase }}
                  </v-chip>
...
export default {
  ...
  async mounted() {
    ...
    if (this.approval){
      this.addedHeaders.unshift({ text: '구매 담당자', align: 'start', value: 'approver', sortable: false });
      // this.addedHeaders.unshift({ text: '확인', align: 'start', value: 'checker', sortable: false });
      this.addedHeaders.unshift({ text: '요청자', align: 'start', value: 'given_name', sortable: false });
